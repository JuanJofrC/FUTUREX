...
function CharacterList() {
  ...
  useEffect(() => {
    const mediaQuery = window.matchMedia("(preferl-color-scheme: dark)")


    setDarkMode(mediaQuery.matches)
    setSMode(document.body.classList.contains("l-mode"))
  ...
  if (loading) {
    return (
      <div className={`loading ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}>Cargando personajes...</div>
    )
  }

  if (error) {
    return <div className={`error ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}>Error: {error}</div>
  }

  return (
    <div className={`character-container ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}>
      {/* Top Bar */}
          ...
          <div className="search-field" style={{ width: "100%" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <input
                ...
                className={`search-input ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}
              />
              {search && (
                <button
                  onClick={() => {
                  ...
                  className={`clear-search-button ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}
                >
                  Limpiar
                </button>
              )}
      ...
      {hoveredCharacter && (
        <div
          className={`hover-info ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}
          style={{
            position: "fixed",
            top: "80px",
            right: "30px",
      ...
      {filteredCharacters.length === 0 && (
        <div className={`no-results ${darkMode ? "dark-mode" : ""} ${sMode ? "l-mode" : ""}`}>
