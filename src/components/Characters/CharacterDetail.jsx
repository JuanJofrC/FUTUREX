...
function CharacterDetail() {
  ...
  useEffect(() => {
    ...
    const islMode = document.body.classList.contains("l-mode")
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.attributeName === "class") {
          ...
          setlMode(document.body.classList.contains("l-mode"))
        }
      })
    })
  ...
  if (loading)
    return (
      <div className={`loading ${darkMode ? "dark-mode" : ""} ${lMode ? "l-mode" : ""}`}>Cargando personaje...</div>
    )
  if (error) return <div className={`error ${darkMode ? "dark-mode" : ""} ${lMode ? "l-mode" : ""}`}>{error}</div>

  return (
    <div className={`character-detail ${darkMode ? "dark-mode" : ""} ${lMode ? "l-mode" : ""}`}>
      <h1>
